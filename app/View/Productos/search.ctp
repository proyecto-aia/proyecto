        ...
        <td>
            <div>
                <fieldset><legend><h3> Buscar Productos/s </h3></legend>
                    <?php
                    echo $this->Form->create('Producto', array('action' => 'search', 'id' => 'search'));
                    ?>
                    <table class='tabla_normal'>
                        <tr>
                            <td>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                echo $this->Form->input('moneda_id', array('label' => 'Tipo de Moneda', 'type' => 'select', 'empty' => __(' ', true), 'options' => $monedas, 'id' => 'moneda_id'));
                                ?>
                            </td> 

                            <td>
                                <?php
                                echo $this->Form->input('titulo', array('label' => 'T&iacute;tulo', 'id' => 'titulo', 'size' => '9'));
                                ?>
                            </td>                            
                        </tr>		
                    </table>
            ...
            if ($buscar == 1) {
                ...
                <div>
                    <fieldset><legend><h3>Resultado/s de la busqueda</h3></legend>
                        <?php
                        echo $this->Form->create('Producto', array('action' => 'delete', 'id' => 'form_producto', 'onsubmit' => 'return verificar_checkbox_vacio();'));
                        ...
                        foreach ($productos_encontrados as $i) {
                            $cells = array(
                                $this->Form->checkbox('Producto.delete.' . $i['Producto']['id']),
                                $this->Html->image('../img/icons/Magnifier.png', array('url' => array('action' => 'view', $i['Producto']['id']), 'class' => 'button1', 'title' => 'Ver')),
                                $this->Html->image('../img/icons/Pencil.png', array('url' => array('action' => 'edit', $i['Producto']['id']), 'class' => 'button1', 'title' => 'Editar')),
                                date('d-m-Y', strtotime($i['Producto']['fecha'])),
                                $i['Producto']['titulo'],
                                $i['Producto']['codigo'],
                                $i['Producto']['status'],
                                $this->Html->image('../img/icons/Pictures.png', array('url' => array('controller' => 'images', 'action' => 'index', 'Producto', $i['Producto']['id']), 'class' => 'button1', 'title' => 'Imagenes')),
                                $this->Html->image('../img/icons/Email.png', array('url' => array('action' => 'pre_send', $i['Producto']['id']), 'class' => 'button1', 'title' => 'Enviar')),
                            );
