...
<table class='tabla_normal'>
	<tr>	
		...
		<?php
		if (!empty($videos)) {	
        echo $form->create('Video', array('action' => 'delete', 'id' => 'form_video', 'onsubmit'=>'return verificar_checkbox_vacio();'));
		...
		foreach($videos as $i) {
			$cells = array(
				$form->checkbox('Video.delete.' . $i['Video']['id'], array('class' => 'check-item')),
                $html->link($this->Html->image('../img/icons/Magnifier.png'), array('action' => 'view', $i['Video']['id']), array( 'class' => 'button1', 'title' => 'Ver')),
                $html->link($this->Html->image('../img/icons/Pencil.png'), array('action' => 'edit', $i['Video']['id']), array( 'class' => 'button1', 'title' => 'Editar')),
